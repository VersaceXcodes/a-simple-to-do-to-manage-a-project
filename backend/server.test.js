...
const db = require('../db'); // import your database connection module (for transactions/cleanup)
const {
...
async function clearTestUser(testUserId) {
  ...
  await db.query('DELETE FROM tasks WHERE user_id = $1', [testUserId]);
  ...
  describe('User Schemas', () => {
    it('should pass with valid createUserInputSchema data', () => {
      const validData = {
        ...
        password_hash: 'longenoughpassword'
      };
      const parsed = createUserInputSchema.parse(validData);
      expect(parsed).toMatchObject({ email: validData.email, password_hash: validData.password_hash, is_demo: false });
    });
...
describe('Integration Tests', () => {
  ...
  beforeAll(async () => {
    ...
    const signupRes = await request(app)
      ...
      .send({ email: testUserEmail, password_hash: testUserPassword });
  ...
  afterAll(async () => {
    ...
    // Optionally end db connection if needed.
    await db.end();
  });
    ...
    describe('POST /api/auth/signup', () => {
      it('should create a new user with valid payload', async () => {
        ...
        const res = await request(app)
          .post('/api/auth/signup')
          .send({ email, password_hash: 'anothergoodpassword' });
      ...
      it('should return 400 when email is invalid', async () => {
        const res = await request(app)
          .post('/api/auth/signup')
          .send({ email: 'invalid-email', password_hash: 'anothergoodpassword' });
      ...
      it('should return 400 when password is too short', async () => {
        const res = await request(app)
          .post('/api/auth/signup')
          .send({ email: 'shortpass@example.com', password_hash: '123' });
        expect(res.status).toBe(400);
      });
      ...
      it('should return 400 for duplicate email', async () => {
        ...
        const res = await request(app)
          .post('/api/auth/signup')
          .send({ email: testUserEmail, password_hash: 'anothergoodpassword' });
        expect(res.status).toBe(400);
      });
    });
